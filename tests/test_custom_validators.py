...
class TestCustomStringValidator:
    def test_startswith_validator_false(self, validator):
        ...
        validator.add_validator("string", "startWith", fn)
    ...
    def test_multiple_custom_string_validators(self, validator):
        startswith_fn = lambda value, start: value.startswith(start)
        endswith_fn = lambda value, end: value.endswith(end)

        validator.add_validator("string", "startWith", startswith_fn)
        validator.add_validator("string", "endsWith", endswith_fn)

        schema = validator.string().test("startWith", "He").test("endsWith", "et")
        assert schema.is_valid("Hexlet") is True
...
class TestCustomNumberValidator:
    def test_min_validator_false(self, validator):
        ...
        validator.add_validator("number", "min", fn)
    ...
    def test_max_validator(self, validator):
        fn = lambda value, max: value <= max
        validator.add_validator("number", "max", fn)
    ...
    def test_range_validator(self, validator):
        fn = lambda value, args: args[0] <= value <= args[1]
        validator.add_validator("number", "inRange", fn)
...
class TestCustomValidatorIsolation:
    ...
    def test_different_validator_instances_have_separate_custom_validators(self):
        v1 = Validator()
        v2 = Validator()

        fn1 = lambda value, start: value.startswith(start)
        fn2 = lambda value, end: value.endswith(end)

        v1.add_validator("string", "startWith", fn1)
        v2.add_validator("string", "endsWith", fn2)
